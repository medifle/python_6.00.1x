...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        LstA1 = self.allParent(node_a)
        LstB1 = self.allParent(node_b)
        LstA2 = []
        LstB2 = []
        for i in LstA1:
            if i in LstB1:
                LstA2 = LstA1[:LstA1.index(i)]
                LstB2 = LstB1[:LstB1.index(i)]
                break
        # if a and b are the same node
        if node_a is node_b:
            cousinType = -1
        # if either one is a direct descendant of the other
        elif node_a in self.allParent(node_b) or node_b in self.allParent(node_a):
            cousinType = -1
        else:
            cousinType = min(len(LstA2), len(LstB2)) - 1
        degreeRemoved = abs(len(LstA2) - len(LstB2))
        return (cousinType, degreeRemoved)
        
    def allParent(self, child):
        '''child is node'''
        if child.get_parent() == None:
            return [child]
        else:
            return [child] + self.allParent(child.get_parent())
