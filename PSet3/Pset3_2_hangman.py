...
WORDLIST_FILENAME = "/Users/bolo/githubRepo/python_6.00.1x/PSet3/words.txt"
...
def isWordGuessed(secretWord, lettersGuessed):
    ...
    for i in secretWord:
        if i not in lettersGuessed:
            check = 0
    # when word has not been guessed
    if check == 0:
        return False
    # when word has been guessed
    else:
        return True
...
def getAvailableLetters(lettersGuessed):
    ...
    # delete guessed letters in listAlpha(list)
    for i in lettersGuessed:
        if i in listAlpha:
            del listAlpha[listAlpha.index(i)]
...
def hangman(secretWord):
    ...
    * After each round, you should also display to the user the 
    ...
    while mistakesMade < 8 and isWordGuessed(secretWord, lettersGuessed) == False:
        ...
        # assumes users will only ever submit one character at a time
