...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        # convert word from string to dict
        wordDict = {}
        for k in word:
            wordDict[k] = wordDict.get(k, 0) + 1
        # check if self.hand could construct the word
        check = 1
        for k in wordDict:
            if not k in self.hand or wordDict[k] > self.hand[k]:
                check = 0
        # update self.hand and return Boolean value if check procedure passes
        if check == 1:
            for k in wordDict:
                self.hand[k] -= wordDict[k]
            return True
        else:
            return False
...
# --Alternative solution for update() START--
        ...
        self.hand = new_hand
        return True
# --Alternative solution for update() END--